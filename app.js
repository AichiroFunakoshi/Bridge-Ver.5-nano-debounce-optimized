// リアルタイム音声翻訳 - GPT‑5 対応版（Responses API / 思考要約は無視 / 文字列のみ取り込み）
...
- 出力は翻訳文のみ。前置き・説明・思考・理由・ラベル・括弧・見出し等は一切出力しない`;
    ...
    function buildResponsesPayload(text) {
        ...
        // GPT‑5 では verbosity / reasoning_effort が利用可能（低めで冗長説明を抑制）
        return {
            model: "gpt-5-nano",
            ...
            input: text,
            stream: true,
            // 追加の抑制パラメータ（公式Cookbook参照）
            verbosity: "low",
            reasoning_effort: "low",
            // 低温度で決定性を高める
            temperature: 0.0,
            // ここでは思考サマリー等は要求しない（Responses API は typed events を返す）
            // reasoning: { summary: "none" } // 明示的に送らない
        };
    }
